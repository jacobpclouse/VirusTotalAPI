...
def writeOutToFile(outgoingData,filenamePrefix):
    ...
    with open(f'{filenamePrefix}.json', 'w') as z:
...
#     url = f"https://www.virustotal.com/vtapi/v2/domain/report?apikey={api_key}&domain={removed_front}"

#     # get domain info from api, send data and key
#     headers = {"Accept": "application/json"}
#     response = requests.get(url, headers=headers)
#     response_json = json.loads(response.content)
